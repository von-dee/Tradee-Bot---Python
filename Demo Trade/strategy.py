...
class Strategy:
    ...
    def exit(self, parent, info = None, from_entry = None, profit = None, loss = None, trail_points = None, trail_offset = None):
        ...
        while True:
            ...
            change_trail = precent_change(parent.orderfill_price, self.trailing_price)
    ...
    def end_trade(self, parent):
        ...
        def precent_profit(previous, current):
            if current == previous:
                change = 0
            try:
                change = (abs(previous - current) / previous) * 100.0
            except ZeroDivisionError:
                change = 0

            if parent.islong == True:
                if (parent.currentposition == 'BUY') and (parent.orderfill_price > self.last_price):
                    change_profit = change*-1
                else:
        ...
        self.trade_data['Profit'] = precent_profit(float(self.trade_data['Entry Price']), float(self.trade_data['Exit Price']))

        act_type = self.trade_data['Exit Signal'] + " Exit"
        title = self.trade_data['Exit Type']
        description = "Sold at price "+ str(self.trade_data['Exit Price']) + " and time " + str(self.trade_data['Exit Date/Time'])
        parent.helper.log_activity(self, len(self.dataclose)-2, 'SELL', act_type, title, description, parent.strategy_code)
