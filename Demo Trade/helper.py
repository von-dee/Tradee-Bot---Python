...
class Helper:
    ...
    def get_minscheck(self, minscheck):

        now = time.ctime()
        
        future = time.time() + minscheck
        date_string = time.ctime(future)
    ...
    def log_activity(self, parent, id, position, act_type, title, description, strategy_code):
